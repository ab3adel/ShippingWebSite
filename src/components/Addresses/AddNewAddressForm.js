...
const AddNewAddressForm = ({ setSliderHeightTrigger, sliderHeightTrigger, refreshRecipint, reseter, countries, handlePrev }) => {
    ...
    useEffect(() => {
        ...
        setCity('')
        cities.length == 0 && fetchCities(117)
    }, [reseter])
    ...
    const onSubmitRecipient = async (values) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                const timer = setTimeout(() => { setSuccessAdd2(''); handlePrev() }, 5000);
